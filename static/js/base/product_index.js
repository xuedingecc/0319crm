...
// 打开对话框
function open_product_dialog(flag) {
    // 赋值1或者2
    $('#flag').val(flag);
...
function delete_product() {
    ...
    $.messager.confirm('系统提示', '您确认想要删除这 ' + ids.length + ' 条记录吗？', function (r) {
        if (r) {
            ...
            // 发送ajax请求
            $.get('/base/delete_product/?id=' + product_ids, function (result) {
                if (200 == result.code) {
                    $.messager.show({
                        title: '系统提示',
                        msg: result.msg,
